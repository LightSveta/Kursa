...
void WriteFile()// запускается 1 раз для создания исходных данных
{
	...
	5 время прерывания 
	число тактов  (на которое прерывается программа после прерывания)
	...
	*/
	...
	int SIZE = 4;//общее число поступивших команд
	WriteSize(SIZE);
	...
	do {
		...
		//rnd = rand() % 500 + 100;//[10;100]//те команды поступают каждые 10-100 тактов
		//
